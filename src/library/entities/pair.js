...
export class Pair {
	...
	getOutputAmount(inputAmount, chainId) {
		...
		const inputAmountWithFee = JSBI.multiply(inputAmount.raw, _997); //: check and fix fees
	...
	getLiquidityMinted(totalSupply, tokenAmountA, tokenAmountB) {
		...
		if (JSBI.equal(totalSupply.raw, ZERO)) {
			...
			liquidity = JSBI.lessThanOrEqual(amount0, amount1)
				? amount0
				: amount1;
		}
	...
	getLiquidityValue(token, totalSupply, liquidity, feeOn = false, kLast) {
		...
		if (!feeOn) {
			...
			if (!JSBI.equal(kLastParsed, ZERO)) {
				const rootK = sqrt(
					JSBI.multiply(this.reserve0.raw, this.reserve1.raw)
				);
